...
function startPromilleBerechnung() {
  ...
  checkUsernameExists(username).then(exists => {
    if (exists) {
      ...
      // ➤ Erlaubnis für Benachrichtigungen anfragen
      if ("Notification" in window && Notification.permission !== "granted") {
        Notification.requestPermission().then(permission => {
          console.log("Push-Berechtigung:", permission);

          if (permission === "granted") {
            // Firebase Messaging laden und Token holen
            import("https://www.gstatic.com/firebasejs/9.22.2/firebase-messaging-compat.js").then(() => {
              ...
              .then((token) => {
                if (token) {
                  ...
                  db.ref("tokens/" + safeName).set({ token });
...
function sendInvitationToDrink() {
  const user = JSON.parse(localStorage.getItem("userData") || "{}");
  const name = user.username || "Jemand";

  fetch("https://us-central1-promille-b4bd3.cloudfunctions.net/sendGlobalPush", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name })
  })
    .then(res => res.text())
    .then(msg => console.log("Push gesendet:", msg))
    .catch(err => console.error("Fehler beim Push senden:", err));
}
